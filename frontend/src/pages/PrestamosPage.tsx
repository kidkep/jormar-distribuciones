...
export function PrestamosPage() {
  ...
  const { data: resumen, isLoading: loadingResumen } = useQuery({
    ...
    queryFn: prestamosApi.getResumen,
  });
  ...
  const createMutation = useMutation({
    mutationFn: prestamosApi.create,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["prestamos"] });
      queryClient.invalidateQueries({ queryKey: ["prestamos-resumen"] });
      queryClient.invalidateQueries({ queryKey: ["caja"] });
      queryClient.invalidateQueries({ queryKey: ["expenses"] });
      setShowForm(false);
      setError(null);
      setForm({ person_name: "", amount: "", distribution_category: "utilidad", payment_method: "efectivo", description: "", reference: "", notes: "" });
    },
    onError: (err: any) => {
  ...
  const pagoMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => prestamosApi.registrarPago(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["prestamos"] });
      queryClient.invalidateQueries({ queryKey: ["prestamos-resumen"] });
      queryClient.invalidateQueries({ queryKey: ["caja"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: prestamosApi.delete,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["prestamos"] });
      queryClient.invalidateQueries({ queryKey: ["prestamos-resumen"] });
      queryClient.invalidateQueries({ queryKey: ["caja"] });
    ...
    <div className="space-y-6">
      ...
      <div className="flex animate-fade-up items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900">
            <Landmark className="h-6 w-6 text-gold-600" />
            Prestamos Internos
          </h1>
          <p className="mt-1 text-sm text-gray-600">Prestamos a vendedores, gerente y CEO</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 animate-fade-up-delay-1">
        <div className="rounded-xl border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-5">
          ...
          <p className="text-xs text-gray-500 mt-1">{r?.prestamos_activos || 0} activos, {r?.prestamos_pagados || 0} pagados</p>
        </div>
      ...
      {showForm && (
        <div className="card-premium animate-scale-in rounded-xl border-2 border-gold-200 bg-gold-50/50 p-6">
          <h3 className="mb-4 text-base font-semibold text-gray-800">Nuevo Prestamo</h3>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Sale de *</label>
              <select
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Metodo de pago *</label>
              <select
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="input-premium"
                placeholder="Ej: Para comprar chicharrones, Para pago de servicios..."
              />
            </div>
            <div>
              ...
              <div key={p.id} className="card-premium animate-fade-up rounded-xl border p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${catColors[p.distribution_category] || ""}`}>
                        {catLabels[p.distribution_category] || p.distribution_category}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 mb-1">{p.description}</p>
                    <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                      <span>Creado: {p.created_at}</span>
                      <span> Metodo salida: {methodLabels[p.payment_method] || p.payment_method}</span>
                ...
                {p.pagos && p.pagos.length > 0 && (
                  <div className="mt-3 rounded-lg bg-gray-50 p-3">
                    <p className="mb-2 text-xs font-medium text-gray-600">Pagos ({p.pagos.length})</p>
                ...
                <div className="mt-3 flex items-center gap-2">
                  {p.status === "activo" && (
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-green-700 transition-all"
                    >
                      <DollarSign className="h-3 w-3" />
                      {showPagoForm === p.id ? "Cancelar" : "Registrar Abono"}
                    </button>
                  )}
                ...
                {showPagoForm === p.id && (
                  <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4">
                    ...
                    <button
                      ...
                      className="mt-3 w-full rounded-lg bg-green-600 py-2 text-sm font-medium text-white hover:bg-green-700 disabled:opacity-50 transition-all"
                    >
                      {pagoMutation.isPending ? "Registrando..." : "Registrar Abono"}
                    </button>
                    {error && (
                      <div className="mt-2 flex items-center gap-2 rounded-lg border border-red-200 bg-white p-2 text-xs text-red-700">
