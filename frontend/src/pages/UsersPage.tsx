      ...
      <div className="flex animate-fade-up items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900">
            <UserCog className="h-6 w-6 text-gold-600" />
            Usuarios
          </h1>
                ...
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(u)} className="rounded-lg p-1.5 text-gold-600 transition hover:bg-gold-50">
                        <Edit className="h-4 w-4" />
                      </button>
                      <button onClick={() => deleteMutation.mutate(u.id)} className="rounded-lg p-1.5 text-red-600 transition hover:bg-red-50">
          ...
          <div className="modal-content w-full max-w-lg rounded-2xl p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} className="input-premium" required />
              </div>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Nombre completo</label>
                  <input type="text" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="input-premium" required />
                </div>
                <div>
              ...
              <div>
                ...
                <select value={form.role_id ?? ""} onChange={(e) => setForm({ ...form, role_id: e.target.value ? Number(e.target.value) : null })} className="input-premium">
