...
async def lifespan(app: FastAPI):
    async with engine.begin() as conn:
        ...
        await conn.execute(text(
            "ALTER TABLE products ADD COLUMN IF NOT EXISTS model_url VARCHAR(500)"
        ))
        await conn.execute(text(
...
("productos.toggle_status", "Activar/Desactivar productos", "productos"),
                ...
                ("reportes.descargar", "Descargar informe en Word", "reportes"),
                ("tareas.view", "Ver tareas y recordatorios", "tareas"),
                ("tareas.gestionar", "Gestionar tareas y recordatorios", "tareas"),
                ("dotacion.view", "Ver tallas y dotacion", "dotacion"),
                ("dotacion.create", "Crear empresas, personas y tallas", "dotacion"),
                ("dotacion.edit", "Editar tallas y dotacion", "dotacion"),
                ("dotacion.delete", "Eliminar tallas y dotacion", "dotacion"),
